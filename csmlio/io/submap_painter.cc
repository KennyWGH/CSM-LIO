...
#include "csmlio/lio/2d/submap_2d.h"
...
Eigen::Affine3d ToEigen(const ::csmlio::transform::Rigid3d& rigid3) {
...
void FillSubmapSlice(
  ...
  ::csmlio::mapping::proto::SubmapQuery::Response response;
  ...
  if (proto.has_submap_3d()) {
    ...
    ::csmlio::mapping::Submap2D submap(proto.submap_2d(),
                                             conversion_tables);
    local_pose = submap.local_pose();
    submap.ToResponseProto(global_submap_pose, &response);
  }
  ...
  submap_slice->slice_pose =
      ::csmlio::transform::ToRigid3(texture_proto.slice_pose());
...
SubmapTexture::Pixels UnpackTextureData(const std::string& compressed_cells,
  ...
  std::string cells;
  ::csmlio::common::FastGunzipString(compressed_cells, &cells);
