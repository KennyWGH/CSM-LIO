...
#ifndef INFINITYSLAM_CSMLIO_HYBRID_GRID_H_
#define INFINITYSLAM_CSMLIO_HYBRID_GRID_H_
...
class NestedGrid {
 public:
  ...
  ValueType* mutable_value(const Eigen::Array3i& index) {
    ...
    if (meta_cell == nullptr) {
      meta_cell = boost::make_unique<WrappedGrid>();
    }
    const Eigen::Array3i inner_index =
        index - meta_index * WrappedGrid::grid_size();
...
#endif  // INFINITYSLAM_CSMLIO_HYBRID_GRID_H_
