...
#ifndef CSMLIO_MAPPING_INTERNAL_3D_SCAN_MATCHING_OCCUPIED_SPACE_COST_FUNCTION_3D_H_
#define CSMLIO_MAPPING_INTERNAL_3D_SCAN_MATCHING_OCCUPIED_SPACE_COST_FUNCTION_3D_H_

#include "Eigen/Core"
#include "csmlio/lio/3d/hybrid_grid.h"
#include "csmlio/lio/internal/3d/scan_matching/interpolated_grid.h"
  ...
  bool Evaluate(const transform::Rigid3<T>& transform,
    ...
    for (size_t i = 0; i < point_cloud_.size(); ++i) {
      const Eigen::Matrix<T, 3, 1> world =
          transform * point_cloud_[i].position.cast<T>(); // wgh-- 问：为什么这里Eigen可以直接cast为Jet?? 
      const T probability =
...
#endif  // CSMLIO_MAPPING_INTERNAL_3D_SCAN_MATCHING_OCCUPIED_SPACE_COST_FUNCTION_3D_H_
