...
#include "infinityslam/csmlio/internal/3d/scan_matching/fast_correlative_scan_matcher_3d.h"
...
#include "infinityslam/csmlio/internal/3d/scan_matching/low_resolution_matcher.h"
...
FastCorrelativeScanMatcher3D::FastCorrelativeScanMatcher3D(
    ...
    : OpTions_(options),
      ...
      precomputation_grid_stack_(
          boost::make_unique<PrecomputationGridStack3D>(hybrid_grid, options)),
...
FastCorrelativeScanMatcher3D::MatchWithSearchParameters(
  ...
  if (best_candidate.score > min_score) {
    return boost::make_unique<Result>(Result{
    ...
    // wgh-- log when failed!
    // wgh-- Update: No need to log here, since it returns min_score whatever.
    LOG(WARNING) << "{Brand & Bound search score " << best_candidate.score 
                 << ", required min_score " <<  min_score << "}";
  }
  return nullptr;
}
    ...
    float min_score) const 
{
  ...
  // best_high_resolution_candidate.score = 0.1; // wgh-- by this way, we will see the actual score in outer logger.
  // wgh-- 此处的得分赋值为min_score，本身就包含了分支定界的要求：所有得分小于这个的branch直接被剪枝差；
  // wgh-- 如果这里赋值为零，因为所有的分支得分都必定大于零，意味着所有的分支都不会被剪掉，就是去了分支定界加速的意义。
  for (const Candidate3D& candidate : candidates) {
    if (candidate.score <= min_score) {
      break;
    }
