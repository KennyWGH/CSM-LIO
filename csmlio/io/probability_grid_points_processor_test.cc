...
#include "csmlio/common/internal/testing/lua_parameter_dictionary_test_helpers.h"
#include "csmlio/common/lua_parameter_dictionary.h"
#include "csmlio/common/port.h"
#include "csmlio/io/fake_file_writer.h"
#include "csmlio/io/points_processor_pipeline_builder.h"
...
::csmlio::io::FileWriterFactory CreateFakeFileWriterFactory(
  ...
  return [&fake_file_writer_output,
    ...
    return ::absl::make_unique<::csmlio::io::FakeFileWriter>(
        full_filename, fake_file_writer_output);
  };
}

std::vector<std::unique_ptr<::csmlio::io::PointsProcessor>>
CreatePipelineFromDictionary(
  ...
  auto builder =
      ::absl::make_unique<::csmlio::io::PointsProcessorPipelineBuilder>();
  builder->Register(
...
std::unique_ptr<common::LuaParameterDictionary> CreateParameterDictionary() {
  auto parameter_dictionary =
      csmlio::common::LuaParameterDictionary::NonReferenceCounted(
          R"text(
          pipeline = { 
            { 
          ...
          absl::make_unique<csmlio::common::DummyFileResolver>());
  ...
  void Run(const std::string& expected_filename) {
    ...
    do {
      pipeline.back()->Process(CreatePointsBatch());
             ...
             csmlio::io::PointsProcessor::FlushResult::kRestartStream);
  }

  std::shared_ptr<std::vector<char>> fake_file_writer_output_ =
      std::make_shared<std::vector<char>>();
  std::unique_ptr<csmlio::common::LuaParameterDictionary>
